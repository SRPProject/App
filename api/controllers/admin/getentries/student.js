...
const getStudentScholarship = async (req, res) => {
  try {
    ...
    if (rq.batchId && rq.degreeDegid && rq.distDepartmentDeptid) {
      //should be in req.query parameters
      ...
      for (let i = 0; i < getScholarshipReport[0].length; i++) {
        getScholarshipReport[0][i]["fileurl"] = await azureBlobSaSUrl(
          "scholarshiproofs",
          getScholarshipReport[0][i]["fileurl"],
        );
      }
  ...
  try {
    ...
    if (rq.batchId && rq.degreeDegid && rq.distDepartmentDeptid) {
      ...
      for (let i = 0; i < getProjectsReport[0].length; i++) {
        getProjectsReport[0][i]["fileurl"] = await azureBlobSaSUrl(
          "projectsproofs",
          getProjectsReport[0][i]["fileurl"],
        );
      }
