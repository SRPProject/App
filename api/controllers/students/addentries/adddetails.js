...
const addintern = async (req, res) => {
  try {
    ...
    if (checkExists) {
      ...
      // const t = await studentVerification.

      (entry.details = req.body.details), await Internships.create(entry);
...
const addscholarship = async (req, res) => {
  try {
    ...
    //     containerClient("scholarshiproofs").getBlockBlobClient(blobname);
...
const addExtraCourses = async (req, res) => {
  try {
    ...
    if (checkExists) {
      ...
      const blockBlobClient =
        containerClient("extracourseproofs").getBlockBlobClient(blobname);
      const data = req.file.buffer;
      await blockBlobClient.upload(data, data.length);
      entry.certificate = blobname;
...
const addEventHackathon = async (req, res) => {
  try {
    ...
    if (checkExists) {
      ...
      const blobname =
        res.locals._id + "_eventhackathon_" + Date.now() + ".pdf";
      const blockBlobClient = containerClient(
...
const addProjects = async (req, res) => {
  try {
    ...
    if (checkExists) {
      ...
      const blockBlobClient =
        containerClient("projectsproofs").getBlockBlobClient(blobname);
      const data = req.file.buffer;
      await blockBlobClient.upload(data, data.length);
