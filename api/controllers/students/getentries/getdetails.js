...
const getPersonalDetails = async (req, res) => {
  try {
    const stid = res.locals._id;
    const getPersonal = await StuPersonalDetails.findOn({
      where: { studentStId: stid },
    });
    if (getPersonal) {
...
const getScholarship = async (req, res) => {
  try {
    const data = await Scholarship.findAll({
      where: { studentStId: res.locals._id },
    });

    return res.status(200).json({message:data})
    // if (getScholarship) {
    //   const scholarshipPDFURL = await azureBlobSaSUrl(
    //     "scholarshiproofs",
    //     getScholarship.proofname,
    //   );
    //   return res
    //     .status(200)
    //     .send({ message: getScholarship, scholarshippdf: scholarshipPDFURL });
...
const getSubjects = async (req, res) => {
  try {
    ...
    var condt = "";
    const getStdDetails = await Students.findOne({
      where: { st_id: res.locals._id },
    });
...
const getMarkSheet = async (req, res) => {
  try {
    if (req.query.semno) {
      const SemMarkSheet = await MarksheetProofs.findOne({
        where: { studentStId: res.locals._id, semno: req.query.semno },
      });
      if (SemMarkSheet) {
        const marksheetURL = await azureBlobSaSUrl(
          "marksheetproofs",
          SemMarkSheet.marksheetname,
        );
...
const getWorkshops = async (req, res) => {
  try {
    const workDetails = await Workshops.findAll({
      where: { studentStId: res.locals._id },
    });

    if (workDetails) {
      for (let i = 0; i < workDetails.length; i++) {
        workDetails[i]["certificate"] = await azureBlobSaSUrl(
          "workshopcertificates",
          workDetails[i]["certificate"],
        );
      }
...
const getExtraCourses = async (req, res) => {
  try {
    const ExtraCoursesDetails = await ExtraCourses.findAll({
      where: { studentStId: res.locals._id },
    });

    if (ExtraCoursesDetails) {
      for (let i = 0; i < ExtraCoursesDetails.length; i++) {
        ExtraCoursesDetails[i]["certificate"] = await azureBlobSaSUrl(
          "extracourseproofs",
          ExtraCoursesDetails[i]["certificate"],
        );
      }
...
const getPaperPublishing = async (req, res) => {
  try {
    const PaperPublishedDetails = await PaperPublished.findAll({
      where: { studentStId: res.locals._id },
    });
...
const getHigherEducation = async (req, res) => {
  try {
    const HigherEducationDetails = await HigherEducation.findAll({
      where: { studentStId: res.locals._id },
    });
...
const getEventHackathon = async (req, res) => {
  try {
    const EventHackathonDetails = await EventHackathon.findAll({
      where: { studentStId: res.locals._id },
    });
    if (EventHackathonDetails) {
      for (let i = 0; i < EventHackathonDetails.length; i++) {
        EventHackathonDetails[i]["certificate"] = await azureBlobSaSUrl(
          "eventhackathonproofs",
          EventHackathonDetails[i]["certificate"],
        );
      }
  ...
  try {
    ...
    if (ProjectDetails) {
      for (let i = 0; i < ProjectDetails.length; i++) {
        ProjectDetails[i]["certificate"] = await azureBlobSaSUrl(
          "projectsproofs",
          ProjectDetails[i]["certificate"],
        );
      }
