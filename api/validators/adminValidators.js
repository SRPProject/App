...
const addStudValidator = async (req, res, next) => {
  ...
  await body("sex").notEmpty().withMessage("gender field invalid").run(req);

  await body("firstname").notEmpty().withMessage("firstname invalid").run(req);

  await body("lastname").notEmpty().withMessage("firstname invalid").run(req);
...
const addSubjectValidator = async (req, res, next) => {
  ...
  await body("subcode").notEmpty().withMessage("Subject Code Invalid").run(req);
  ...
  await body("typeofsub")
       .isIn(["COMPULSORY","AUDIT","ELECTIVES","HUMANITIES"])
       .withMessage("Type of Subject is not valid")
      .run(req);
...
const addBatchValidator = async (req, res, next) => {
  ...
  await body("endyr")
    ...
    await body("degreeDegid")
    .notEmpty()
    .withMessage("degree is needed")
    .bail()
    .isNumeric()
    .withMessage("degree id is not valid")
    .run(req);

    await body("regulationRegid")
    .notEmpty()
    .withMessage("regulation id is needed")
    .bail()
    .isNumeric()
    .withMessage("regulation id is not valid")
    .run(req);

  await body("endyr")
    .custom(async (value, { req }) => {
      if (req.body.endyr && req.body.startyr) {
        if (Number(req.body.endyr) <= Number(req.body.startyr)) {
          throw new Error("End year should be greater than start year");
        }
      }
    })
    .run(req);
