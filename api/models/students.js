...
const StudentSem = sequelize.define(
  "studentsems",
  {
...
const Placement = sequelize.define("placements", {
  ...
  comptype: { type: DataTypes.BOOLEAN, defaultValue: "0" }, //service / product
});
...
/*
  relations for verification and data 
*/ 

/*
  explantion on how data is verified :
  1) Login :
        -> Student table has verification id which is foreign key to student_verifications
        -> whenerver student signup for first time create verifcation row 
        -> if faculty verify the student proofs then student_verifcations.status = true and relevant tables are updated
        -> else verifications row is deleted 

  2)  Academics :(semesters)
        -> studentSems store data for each subject of student {subjectid,studentId}
        -> marksheetProof table stores {studentid,semester_no} which is primary key 
        -> marksheetProof also has column 'verifcationId' which is pointed to verification table
        -> thus semester marksheets can be verified by updating verification table via verificationId

  3)  all others :
        -> each table has field called as studentVerifcationId which is foreign key to student_verifcaitons
        -> faculty can update the status similarly like login 
